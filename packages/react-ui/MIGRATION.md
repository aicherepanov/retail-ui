...
Пакет `retail-ui` все стили библиотеки из less в css-in-js, вместе с ним старый механизм кастомизации больше не поддерживается, в следствии чего теряется необходимость дальнейшей поддержки пакета `retail-ui`. В рамках поддержки LTS версий пакет `retail-ui` будет выпускаться, но будет содержать исправления критичных багов.
